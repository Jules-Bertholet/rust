...
### The trait requires `Self: Sized` or `Self: Aligned`

Traits that are declared as `Trait: core::marker::Aligned` or which otherwise
inherit a constraint of `Self: Aligned` are not object-safe.
...
However as we know, any `dyn Trait` is also unsized and unaligned, and so it
can never implement a trait like `Trait: Aligned`. So, rather than allow an
exception to the rule that `dyn Trait` always implements `Trait`, Rust chooses
to prohibit such a `dyn Trait` from existing at all.

Only unsized, unaligned traits are considered object-safe.

Generally, `Self: Sized` or `Self: Aligned` is used to indicate that the trait
should not be used as a trait object. If the trait comes from your own crate,
